...
def get_fast_align_transition(theta, i, j, m, n):
    if j == NULL:
        ...
        print '*'
        """
        return np.log(1 - Po) + (lf * h(i, j, m, n)) - np.log(z_lf(lf, i, m, n))


def get_lambda_feature_grad():
    pass
...
def get_gradient(theta):
    ...
    d = abs(exp(fractional_counts[lambda_event1])) + abs(exp(fractional_counts[lambda_event2]))
    grad_lf = -n
    grad[feature_index[LAMBDA_FEATURE]] += grad_lf
    print grad[feature_index[LAMBDA_FEATURE]], 'is the grad for lambda'
    """
    return -grad
