...
do
    for RC in 0.0
    do
        time python ${CODE_DIR}/featurized_model1.py -s $SOURCE_FULL -t $TARGET_FULL -a $ALGO  --iw initial.trans --ts $SOURCE_TEST --tt $TARGET_TEST -r $RC --df $DICT_PATH
        time python ${CODE_DIR}/featurized_model1_mp.py -s $SOURCE_FULL -t $TARGET_FULL -a $ALGO  --iw initial.trans --ts $SOURCE_TEST --tt $TARGET_TEST -r $RC --df $DICT_PATH
        echo "."
    done
done
